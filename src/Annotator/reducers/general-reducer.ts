...
import { ExpandingLine, moveRegion, Region } from "../../ImageCanvas/region-tools"
...
export default (state: MainLayoutState, action: Action) => {
  ...
  if (action.type === "MOUSE_MOVE") {
    ...
    state = Immutable(state).setIn(["lastMouseMoveCall"], Date.now())
  }
  if (!action.type.includes("MOUSE")) {
    state = Immutable(state).setIn(["lastAction"], action)
  }
  ...
  const getRegion = (regionId: string | number | Region) => {
    if (!activeImage) return null
  ...
  const modifyRegion = (regionId: string | number | Region, obj) => {
  ...
  const setNewImage = (img: string | Image, index: number) => {
    let { src, frameTime } = typeof img === "object" ? img : { src: img, frameTime: undefined }
    return Immutable(Immutable(state).setIn(["selectedImage"], index)).setIn(
  ...
  switch (action.type) {
    ...
    case "CHANGE_REGION": {
      ...
      if (oldRegion.cls !== action.region.cls) {
        ...
        if (clsIndex !== -1) {
          state = Immutable(state).setIn(["selectedCls"], action.region.cls)
    ...
    case "CHANGE_IMAGE": {
      ...
      for (const key of Object.keys(delta)) {
        ...
        state = Immutable(state).setIn([...pathToActiveImage, key], delta[key])
      }
      return state
    }
    case "SELECT_REGION": {
    ...
    case "BEGIN_BOX_TRANSFORM": {
      ...
      if (directions[0] === 0 && directions[1] === 0) {
        return Immutable(state).setIn(["mode"], { mode: "MOVE_REGION", regionId: box.id })
      } else {
        return Immutable(state).setIn(["mode"], {
          mode: "RESIZE_BOX",
          regionId: box.id,
    ...
    case "ADD_POLYGON_POINT": {
      ...
      return Immutable(state).setIn([...pathToActiveImage, "regions", regionIndex], {
        ...polygon,
        points
      })
    }
    case "MOUSE_MOVE": {
      ...
      switch (state.mode.mode) {
        ...
        case "MOVE_KEYPOINT": {
          ...
          const [region, regionIndex] = getRegion(regionId)
          if (regionIndex === null || typeof region !== "object" || !("points" in region)) return state
          return Immutable(state).setIn(
            [
              ...pathToActiveImage,
              "regions",
        ...
        case "RESIZE_BOX": {
          ...
          if (dw <= 0.001) {
            state = Immutable(state).setIn(["mode", "freedom"], [xFree * -1, yFree])
          }
          if (dh <= 0.001) {
            state = Immutable(state).setIn(["mode", "freedom"], [xFree, yFree * -1])
          }
        ...
        case "DRAW_POLYGON": {
          ...
          if (typeof region !== "object" || region.type !== "polygon") return state
          const { points } = region
        ...
        case "DRAW_LINE": {
          const { regionId } = state.mode
          const [region, regionIndex] = getRegion(regionId)
          if (!region || typeof region !== "object") return Immutable(state).setIn(["mode"], null)
          return Immutable(state).setIn([...pathToActiveImage, "regions", regionIndex], {
            ...region,
            x2: x,
            y2: y
        ...
        case "DRAW_EXPANDING_LINE": {
          ...
          if (!expandingLine || typeof expandingLine !== "object" || expandingLine.type !== "expanding-line") return state
    ...
    case "MOUSE_DOWN": {
      ...
      state = Immutable(state).setIn(["mouseDownAt"], { x, y })

      if (state.mode) {
        switch (state.mode.mode) {
          case "DRAW_POLYGON": {
            const [polygon, regionIndex] = getRegion(state.mode.regionId)
            if (!polygon || typeof polygon !== "object" || polygon.type !== "polygon") break
            return Immutable(state).setIn(
              [...pathToActiveImage, "regions", regionIndex],
          ...
          case "DRAW_LINE": {
            ...
            Immutable(state).setIn([...pathToActiveImage, "regions", regionIndex], {
              ...line,
              x2: x,
              y2: y
            })
          ...
          case "DRAW_EXPANDING_LINE": {
            ...
            if (!expandingLine || typeof expandingLine !== "object" || expandingLine.type !== "expanding-line") break
            const lastPoint = expandingLine.points.slice(-1)[0]
            if (
      ...
      const regions = [...(Immutable(state).getIn(pathToActiveImage).regions || [])]
        .map((r) =>
          Immutable(r).setIn(["editingLabels"], false).setIn(["highlighted"], false)
        )
    ...
    case "MOUSE_UP": {
      ...
      if (!state.mode) return state
      state = Immutable(state).setIn(["mouseDownAt"], null)
      switch (state.mode.mode) {
        case "RESIZE_BOX": {
          if (state.mode.isNew) {
            if (
        ...
        case "DRAW_EXPANDING_LINE": {
          ...
          if (mouseDistFromLastPoint > 0.002) {
            ...
            for (let i = 0; i < newPoints.length - 1; i++) {
              ...
              newPoints[i] = {
                ...newPoints[i],
                width: "width" in lastPoint ? lastPoint.width as number : 0.002
              }
            }
    ...
    case "OPEN_REGION_EDITOR": {
      ...
      return Immutable(state).setIn([...pathToActiveImage, "regions"], newRegions)
    }
    ...
    case "HEADER_BUTTON_CLICKED": {
      ...
      switch (buttonName) {
        ...
        case "next": {
          ...
          if (currentImageIndex === (state.images.length - 1).toString()) return state
          return setNewImage(
            state.images[+currentImageIndex + 1],
            +currentImageIndex + 1
          )
    ...
    case "SELECT_TOOL": {
      ...
      if (action.selectedTool === "modify-allowed-area" && !state.allowedArea) {
        state = Immutable(state).setIn(["allowedArea"], { x: 0, y: 0, w: 1, h: 1 })
      }
      state = Immutable(state).setIn(["mode"], null)
