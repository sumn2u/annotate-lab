    ...
    t: (key) =>
      ({
        ...
        "configuration.region_types_allowed.description":
          "Select the types of regions allowed",
    ...
    <div data-testid="mocked-survey">
      {form.questions.map((q) => (
        <div key={q.name} data-testid={`question-${q.name}`}>
          {q.title}
          {q.type === "boolean" && (
            <input type="checkbox" data-testid={`checkbox-${q.name}`} />
          )}
...
describe("ConfigureImageSegmentation", () => {
  test("renders form with questions and calls onChange on answer change", () => {
    ...
    const multipleRegionsCheckbox = screen.getByTestId(
      "checkbox-multipleRegions",
    )
    const multipleRegionLabelsCheckbox = screen.getByTestId(
      "checkbox-multipleRegionLabels",
    )
    ...
    fireEvent.change(multipleRegionLabelsCheckbox, {
      target: { checked: true },
    })
    expect(multipleRegionLabelsCheckbox).toBeChecked()

    // Simulate changing multiple-dropdown and verify onChange is called
    const regionTypesAllowedDropdown = screen.getByTestId(
      "multiple-dropdown-regionTypesAllowed",
    )
