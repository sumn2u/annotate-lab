...
describe("HistorySidebarBox", () => {
  ...
  it("renders each history item correctly", () => {
    ...
    render(
      <HistorySidebarBox history={mockHistory} onRestoreHistory={() => {}} />,
    )
    ...
    mockHistory.forEach((item) => {
      ...
      const timeElements = screen
        ...
        .map((listItem) => listItem.querySelector("p"))
      const hasFormattedTime = timeElements.some(
        (p) => p.textContent === formattedTime,
      )
  ...
  it("calls onRestoreHistory when undo icon is clicked for the first history item", () => {
    ...
    const { getByTestId } = render(
      <HistorySidebarBox
        history={history}
        onRestoreHistory={onRestoreHistoryMock}
      />,
    )
