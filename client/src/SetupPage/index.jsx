      ...
      <Box>
        <Box paddingBottom="0px">
          ...
          >
            <Tab
              icon={<CategoryIcon />}
              label={t("setup.tabs.taskinfo")}
              value="datatype"
            />
            <Tab
              disabled={!isTaskInfoComplete}
              icon={<BuildIcon />}
              label={t("setup.tabs.configure")}
              value="configure"
            />
            <Tab
              disabled={!isConfigComplete}
              icon={<AddPhotoAlternateIcon />}
              label={t("setup.tabs.images")}
              value="images"
            />
          </Tabs>
        </Box>
        <IconButton
          disabled={
            !isTaskInfoComplete || !isConfigComplete || !isImagesUploaded
          }
          onClick={showLab}
          sx={{
            position: "absolute",
        ...
        >
          {hasShowLab && hasConfig && (
            <CloseIcon fontSize={isLargeDevice ? "large" : "medium"} />
          )}
        </IconButton>
        {currentTab === "datatype" && (
          <Box minWidth="55vw" paddingTop={"1rem"}>
            <>
              ...
              >
                <Button
                  variant="contained"
                  disabled={settings.taskDescription.trim().length <= 0}
